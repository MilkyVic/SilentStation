...
const OPTIONS_LIKERT_1_5: SeedOption[] = [
...
const OPTIONS_DASS_0_3: SeedOption[] = [
...
const OPTIONS_MBI_0_6: SeedOption[] = [
...
const makeQuestions = (
  items: string[],
  options: SeedOption[],
  subscaleResolver?: (index: number) => string,
...
const resolveSdqSubscale = (index: number) => {
  if ([3, 8, 13, 16, 24].includes(index)) return 'emotional';
  if ([5, 7, 12, 18, 22].includes(index)) return 'conduct';
  if ([2, 10, 15, 21, 25].includes(index)) return 'hyperactivity';
  if ([6, 11, 14, 19, 23].includes(index)) return 'peer';
  return 'prosocial';
};

const resolveMtSubscale = (index: number) => {
  if (index <= 5) return 'safety';
  if (index <= 10) return 'respect';
  if (index <= 17) return 'support';
  if (index <= 22) return 'learning_env';
  return 'mental_health_awareness';
};

const resolveDassSubscale = (index: number) => {
  if ([1, 6, 8, 11, 12, 14, 18].includes(index)) return 'stress';
  if ([2, 4, 7, 9, 15, 19, 20].includes(index)) return 'anxiety';
  return 'depression';
};

const resolveMbiSubscale = (index: number) => {
  if ([1, 2, 3, 6, 8, 13, 14, 16, 20].includes(index)) return 'EE';
  if ([5, 10, 11, 15, 22].includes(index)) return 'DP';
  return 'PA';
};

export const SYSTEM_TEMPLATE_QUESTIONS: Record<string, SeedQuestion[]> = {
