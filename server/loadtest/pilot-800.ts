...
const buildAnswers = (templateId: string) => {
  const id = String(templateId);
  const random = (min: number, max: number) => Math.floor(Math.random() * (max - min + 1)) + min;
  if (id === '1') return Array.from({ length: 9 }, () => random(0, 3));
  if (id === '2') return Array.from({ length: 7 }, () => random(0, 3));
  if (id === '3') return Array.from({ length: 25 }, () => random(0, 2));
  if (id === '4') return Array.from({ length: 20 }, () => random(0, 2));
  if (id === '5') return Array.from({ length: 21 }, () => random(0, 3));
...
const callSubmit = async (token: string): Promise<RequestMetric> => {
...
const callReport = async (token: string): Promise<RequestMetric> => {
  const start = Date.now();
  try {
...
const printSummary = (title: string, metrics: RequestMetric[]) => {
