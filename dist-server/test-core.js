...
import { SYSTEM_TEMPLATE_QUESTIONS } from './test-question-bank.js';
const getAudiencesByRole = (role) => {
    if (role === 'student')
        return ['student', 'both'];
    if (role === 'teacher')
        return ['teacher', 'both'];
    if (role === 'admin' || role === 'superadmin')
        return ['student', 'teacher', 'both'];
    return ['both'];
};
...
const buildSeedQuestionId = (templateId, order) => `${templateId}-q${order}`;
const buildSeedOptionId = (questionId, order) => `${questionId}-o${order}`;
const seedTemplateQuestions = async (templateId, questions) => {
...
const scoreByTemplate = (templateId, templateCode, answers, fallbackScore) => {
    const code = String(templateCode || '').toUpperCase();
    if (code === 'PHQ9') {
        if (!answers.length)
            throw new AuthHttpError(400, 'AUTH_SERVER_ERROR', 'PHQ-9 can du dap an de cham diem.');
    ...
    if (code === 'GAD7') {
        if (!answers.length)
            throw new AuthHttpError(400, 'AUTH_SERVER_ERROR', 'GAD-7 can du dap an de cham diem.');
    ...
    if (code === 'DASS21') {
        if (!answers.length)
            throw new AuthHttpError(400, 'AUTH_SERVER_ERROR', 'DASS-21 can du dap an de cham diem.');
...
const ensureTemplateWritePermission = (userRole, isSystem) => {
...
const normalizeTemplateSnapshot = (row) => ({
    id: row.id,
    title: row.title,
    description: row.description,
    targetAudience: row.target_audience,
...
const appendTemplateVersion = async (templateRow, action, actorUserId) => {
...
export const listTestCatalog = async (token) => {
...
export const listManageTestTemplates = async (token) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
...
export const listTemplateVersions = async (token, templateId) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    if (user.role !== 'admin' && user.role !== 'superadmin') {
...
export const getTestTemplateDetail = async (token, templateId) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    const id = String(templateId || '').trim();
    if (!id) {
...
export const submitTestResult = async (token, payload) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
  ...
  `, [templateId]);
    ...
    const audiences = getAudiencesByRole(user.role);
    if (!audiences.includes(String(template.target_audience || ''))) {
    ...
    const scoreData = scoreByTemplate(templateId, String(template.template_code || ''), answers, Number(body.score));
...
export const listMyTestResults = async (token) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    const result = await getPool().query(`
    SELECT
...
export const getTestReportsOverview = async (token, query) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    ...
    const params = [days];
    const where = [`ta.submitted_at >= NOW() - ($1::int * INTERVAL '1 day')`];
  ...
  `, params);
    ...
    for (const item of attempts) {
        ...
        if (item.className) {
            ...
            if (item.isHighRisk)
                classEntry.highRiskCount += 1;
            byClassMap.set(item.className, classEntry);
        }
    }
    const sortByMonthAsc = (a, b) => a.month.localeCompare(b.month);
...
export const getTestOpsSummary = async (token, query) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    if (user.role !== 'admin' && user.role !== 'superadmin') {
    ...
    const params = [hours];
    const where = [`ta.submitted_at >= NOW() - ($1::int * INTERVAL '1 hour')`];
    if (user.role === 'admin') {
...
export const createCustomTestTemplate = async (token, payload) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    if (user.role !== 'admin' && user.role !== 'superadmin') {
    ...
    if (!title)
        throw new AuthHttpError(400, 'AUTH_SERVER_ERROR', 'Thieu tieu de bai test.');
...
export const updateCustomTestTemplate = async (token, templateId, payload) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    if (user.role !== 'admin' && user.role !== 'superadmin') {
  ...
  `, [templateId]);
    ...
    const action = beforeActive !== afterActive
...
export const deleteCustomTestTemplate = async (token, templateId) => {
    await initializeTestCore();
    const user = await getCurrentUser(token);
    if (user.role !== 'admin' && user.role !== 'superadmin') {
...
export const publishTestTemplate = async (token, templateId, payload) => {
    const body = (payload && typeof payload === 'object') ? payload : {};
