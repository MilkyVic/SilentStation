...
const normalizeForRiskDetection = (input) => (input
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/đ/g, 'd')
    ...
    .replace(/\s+/g, ' ')
    .trim());
const compactForRiskDetection = (input) => normalizeForRiskDetection(input).replace(/\s+/g, '');
const RED_CODE_DIRECT_PATTERNS = [
    /\btu\s*tu\b/,
    /\btu\s*sat\b/,
    /\btu\s*ket\s*lieu\b/,
    /\bquyen\s*sinh\b/,
...
const RED_CODE_BYPASS_COMPACT_PATTERNS = [
    /t\W*u\W*t\W*u/,
    /t\W*u\W*s\W*a\W*t/,
    /t\W*u\W*k\W*e\W*t\W*l\W*i\W*e\W*u/,
];
const RED_CODE_FALSE_POSITIVE_PATTERNS = [
...
const RED_CODE_AMBIGUOUS_SLANG_PATTERNS = [
    /\b44\b/,
    /\breset\b/,
    /\bdang\s*xuat\b/,
    /\bisekai\b/,
...
const RED_CODE_AMBIGUOUS_CONTEXT_PATTERNS = [
    /\bcuoc\s*doi\b/,
    /\bban\s*than\b/,
...
const CHAT_SCOPE_HINT_PATTERNS = [
    /\bhoc\s*sinh\b/,
    /\bthpt\b/,
    /\bhoc\s*duong\b/,
    /\btruong\b/,
...
const detectRedCode = (input) => {
    ...
    if (slangHit) {
        ...
        if (contextHit) {
            return { triggered: true, reason: 'slang_with_context' };
        }
    }
    return { triggered: false, reason: 'none' };
};
const detectOutOfScope = (input, knowledgeChunks) => {
    const normalized = normalizeForRiskDetection(input);
    if (!normalized)
        return false;
    ...
    if (hasScopeHint)
        return false;
    const terms = Array.from(new Set(tokenize(input))).filter((term) => !SCOPE_STOPWORDS.has(term) && term.length >= 3);
    if (!terms.length)
        return false;
    let matchedTerms = 0;
...
const sanitizeText = (value) => (typeof value === 'string' ? value.replace(/\s+/g, ' ').trim() : '');
const buildChunkId = (source, index) => `${source}#${index}`;
const chunkMarkdown = (source, content) => {
...
const buildPrompt = (systemPrompt, message, contextChunks, history, metadata) => {
...
const fallbackAnswer = (message, chunks) => {
    if (!chunks.length) {
...
export const sendChatMessage = async (payload) => {
    ...
    if (redCode.triggered) {
        ...
        session.messages.push({ role: 'user', content: incomingMessage, createdAt: Date.now() }, { role: 'assistant', content: reply, createdAt: Date.now() });
    ...
    const prompt = buildPrompt(systemPrompt, incomingMessage, contextChunks, session.messages, {
        app: 'tram-an',
